...
plot_1 <- ggplot(diag_data) +
    ...
    geom_smooth() +
    ggtitle("A: Predicted values versus the residuals")

plot_2 <- ggplot(diag_data) +
    aes(x = .fitted, y = sqrt(abs(.stdresid))) +
    geom_point(alpha = .5, shape = ".") +
    geom_smooth() +
    ggtitle("B: Predicted values versus the standardised residuals")
...
plot_3 <- ggplot(diag_data) +
    ...
    geom_qq() +
    ggtitle("C: QQ plot of the residuals")

plot_4 <- ggplot(diag_data) +
    aes(x = .stdresid) +
    geom_histogram(bins = 30) +
    geom_freqpoly(data = rnorm(nrow(diag_data), 0, 1) %>% data_frame(x = .), aes(x = x), color = "red", size = 1) +
    ggtitle("D: Distribution of the residuals", subtitle = "The red line shows the normal distribution")
